...
mraa_phyboard_pico()
{
    ...
    /* PIN-Number "4" with name "X_GPIO1_IO04__PWM3_OUT" */
    /*if (uart2_enabled == 1) {
        strncpy(b->pins[20].name, "X_GPIO1_IO4", MRAA_PIN_NAME_SIZE);
        b->pins[20].capabilities = (mraa_pincapabilities_t){ 1, 1, 0, 0, 0, 0, 0, 0 };
        b->pins[20].gpio.pinmap = ((1-1)*32 + 4);
    ...
    strncpy(b->pins[27].name, "X_GPIO1_IO1", MRAA_PIN_NAME_SIZE);
    b->pins[27].capabilities = (mraa_pincapabilities_t){ 1, 1, 0, 0, 0, 0, 0, 1 };
    b->pins[27].gpio.pinmap = ((1-1)*32 + 4);
